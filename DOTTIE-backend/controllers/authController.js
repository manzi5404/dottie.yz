...
const signup = async (req, res) => {
    authLog('âš™ï¸ authController.signup start', { email: req.body.email });
    try {
        ...
        if (existingUser) {
            authLog('âš ï¸ authController.signup user exists', { email });
        ...
        authError('âŒ authController.signup error:', error);
...
const login = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            authLog('âš ï¸ authController.login user not found', { email });
        ...
        if (!isPasswordCorrect) {
            authLog('âš ï¸ authController.login invalid credentials', { email });
        ...
        authError('âŒ authController.login error:', error);
        res.status(500).json({ message: 'Something went wrong', error: error.message });
    }
};
...
const googleLogin = async (req, res) => {
    try {
        ...
        const jwtToken = jwt.sign(
            ...
            process.env.JWT_SECRET || 'dottie_secret',
            { expiresIn: '2h' }
        );
